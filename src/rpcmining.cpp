...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("stakeinterest",    (uint64_t)Params().ProofOfStakeReward()));
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Moonshot is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Moonshot is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Moonshot is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Moonshot is downloading blocks...");
