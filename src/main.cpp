...
#if defined(NDEBUG)
# error "Moonshot cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Moonshot Signed Message:\n";
...
{
    int64_t nSubsidy = nCoinAge * Params().ProofOfStakeReward() * 33 / (365 * 33 + 8);
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal coin miner
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (!txdb.ReadCheckpointPubKey(strPubKey) || strPubKey != CSyncCheckpoint::strMasterPubKey)
    {
        ...
        if ((Params().NetworkIDString() == CChainParams::MAIN) && !Checkpoints::ResetSyncCheckpoint())
