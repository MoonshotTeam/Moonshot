...
{
    QString ret = QString("Moonshot:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non coin unit input in lnReqAmount, we generate the URI with base unit as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(CoinUnits::format(CoinUnits::BASEUNIT, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
