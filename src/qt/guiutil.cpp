...
bool parseCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Moonshot"))
        return false;
...
{
    // Convert Moonshot:// to Moonshot:
    if(uri.startsWith("Moonshot://"))
    {
        uri.replace(0, 12, "Moonshot:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Moonshot.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Moonshot.lnk
...
{
    return GetAutostartDir() / "Moonshot.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Moonshot.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Moonshot\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Moonshot") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Moonshot [" + tr("command-line options") + "]                     " + "\n";
