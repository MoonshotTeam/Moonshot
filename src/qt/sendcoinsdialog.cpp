    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Moonshot address (e.g. iJagcLKkACTYVhyVuDimpU7tW8Xfw6GXQK)"));
#endif
    ...
    // Coin Control
    ui->lineEditCoinControlChange->setFont(GUIUtil::coinAddressFont());
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    {
        formatted.append(tr("<b>%1</b> to %2 (%3)").arg(CoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), rcp.amount), Qt::escape(rcp.label), rcp.address));
    }
    ...
    case WalletModel::AmountWithFeeExceedsBalance:
        QMessageBox::warning(this, tr("Send Coins"),
            ...
            arg(CoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), sendstatus.fee)),
            QMessageBox::Ok, QMessageBox::Ok);
        break;
...
void SendCoinsDialog::setBalance(qint64 balance, qint64 stake, qint64 unconfirmedBalance, qint64 immatureBalance)
{
    ...
    if(model && model->getOptionsModel())
    {
        ui->labelBalance->setText(CoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), balance));
    }
}
...
void SendCoinsDialog::coinControlChangeChecked(int state)
{
    ...
    {
        if (state == Qt::Checked)
            CoinControlDialog::coinControl->destChange = CCoinAddress(ui->lineEditCoinControlChange->text().toStdString()).Get();
        else
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        ...
        else if (!CCoinAddress(text.toStdString()).IsValid())
        {
            ...
            ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid Moonshot address"));
        }
        else
        {
